...
public class CPMPlayer : MonoBehaviour
{
    ...
    private void Update()
    {
        //Debug.Log("CPMPlayer.Update(): transform.localPosition pre-Update(): " + this.transform.localPosition);
        ...
        //rotZ += Input.GetAxisRaw("Mouse Z") * zMouseSensitivity * 0.02f;
        ...
        playerView.position = new Vector3(
            transform.position.x + playerViewYOffset,
            transform.position.y + playerViewYOffset,
            transform.position.z + playerViewYOffset);

        //Debug.Log("CPMPlayer.Update(): transform.localPosition post-Update(): " + this.transform.localPosition);
    }
    ...
    */
    ...
    // FIXME: LH: yeah, we need a better way for this
    public bool PlayAnimation(Animator animatorRef, System.String animationID, bool isTriggerUsed = false, bool triggerState = false)
    {
        if (animationID != String.Empty)
        {
            if (animatorRef)
            {
                int hashedAnimationID = Animator.StringToHash(animationID);

                // For cases, where we want to set triggers (involved in some more complex animation flow), 
                // instead of playing an animation directly, once
                if (isTriggerUsed && (animatorRef.GetBool(hashedAnimationID) != triggerState))
                {
                    animatorRef.SetBool(hashedAnimationID, triggerState);
                    return true;
                }
                else
                {
                    animatorRef.Play(hashedAnimationID);
                    return true;
                }
            }
        }
